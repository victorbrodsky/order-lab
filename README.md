...
The [data model of the key objects is provided in UML and JPG formats](https://github.com/victorbrodsky/order-lab/tree/master/Scanorders2/uml) as part of the distribution.
...
2. [Download](https://github.com/victorbrodsky/order-lab/archive/master.zip) and uncompress or [clone](https://help.github.com/articles/cloning-a-repository/) the source code from [github.com/victorbrodsky/order-lab](https://github.com/victorbrodsky/order-lab)
...
7. Visit http://IPADDRESS/order/directory/admin/first-time-login-generation-init/ in your browser to generate the initial Administrator account, where IPADDRESS is either the IP address of the server, or the domain name. Then, log into the application with the user name "Administrator" and the password "1234567890" at http://IPADDRESS/order/. Change the default password for the Administrator account by visiting the account's profile page and clicking 'Edit'.
...
10. Obtain and install these optional applications to enable associated functionality on the server:
...
11. If desired, download the template file for bulk user data import, fill it out with user details, and upload it back to the site via the import function.
...
3. ['Homebrew'](https://brew.sh) can be used to install the necessary software: [Packer](https://www.packer.io/) and [doctl](https://github.com/digitalocean/doctl). This can be performed through the Terminal application.
...
4. Edit /ORDER_LOCATION/order-lab-master/packer/parameters.yml in this project's folder to set desired values (especially for passwords) using a text editor such as TextEdit and save.
...
## Installation Instructions for deploying a Windows-based server on Windows (Tested on Windows 10 and Windows 7)
...
2. Configure AMPPS. At this point, AMPPS should be open, and Apache web server, PHP, and MySQL should display as "Running". If Apache or MySQL is not running, turn it on by clicking the switch to the left of it's name in the AMPPS tray menu.
	...
	c) If an option other than AMPPS is being used, the following configurations may be necessary. If AMPPS is being used, skip this step.
		...
		If you are using MSSQL Server, you will need to download the [Microsoft ODBC Driver 11 for SQL Server](https://www.microsoft.com/en-us/download/details.aspx?id=36434) for PHP to access the database:
	...
	d) You may need to download and enable OPcache if you are not using AMPPS (in which it is already enabled by default). Note: this step is required if OPcache is not running. PHP configuration can be verified on the http://IPADDRESS/order/order/info.php page after step 3.


		a) For PHP version 5.4 [download OPcache](http://windows.php.net/downloads/pecl/releases/opcache/7.0.3/php_opcache-7.0.3-5.4-ts-vc9-x86.zip) and [enable it](http://stackoverflow.com/questions/24155516/how-to-install-zend-opcache-extension-php-5-4-on-windows); for PHP version 5.6 just [enable OPCache](http://php.net/manual/en/opcache.setup.php) by adding this line to php.ini:
...
10. If desired, download the template file for bulk user data import, fill it out with user details, and upload it back to the site via the import function.
