...
//            echo "messageCategory=".$messageCategory->getName()."<br>";
    ...
    public function findAndReplaceOldByNew( $entityName, $oldNumberId, $newNumberId ) {
        $em = $this->getDoctrine()->getManager();
        ...
        $dql->where("numberid.number LIKE :oldNumberId");
        ...
        foreach( $numberObjects as $numberObject ) {
            ...
            if( count($fieldArr) != 2 ) {
                throw new \Exception( 'Must have 2 parts object. Found '.count($params).'object(s)' );
                continue;
            }
            $fieldId = $fieldArr[1];
