...
function addCollectionField( elem ) {
    ...
    //var collInputs = elementHolder.find('input[type=text]');
    //var collInputs = elementHolder.find('input[type=text]');
    ...
    //exception for slide; make it as array field later on
    if( name == "slide" ) {
        ident = fieldName;
        var prefix = "";
    }
    ...
    var lastcollHolder = collHolders.eq(collHoldersCount-1);
    var createdElement = lastcollHolder.after(newForm);

    var btnDel = getDelBtn();
...
function delCollectionField( elem ) {

    var element = $(elem);
    var holder = element.parent().parent().parent().parent().parent().parent();

    var elementHolder = element.parent().parent().parent().parent().parent();
    elementHolder.remove();

    var elementsEnabled = holder.find('input[type=text]:enabled:not([readonly])');
