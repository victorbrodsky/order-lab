...
send_alert()
{
   bash $script_full_path/alert_dba -FALERT -S"PostgreSQL backup failed" -B"$LOG" -P"$PROG" -AY -GN -C"$HOSTNAME"
}
...
if [ ! -f ${PGDATA}/postmaster.pid ]; then
   ...
   bash $script_full_path/alert_dba -FALERT -S"PostgreSQL database is down" -B"PostgreSQL database is down in this machine $HOSTNAME" -P"$PROG" -AY -GN -C"$HOSTNAME"
   exit
else
