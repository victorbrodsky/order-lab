...
class DataQualityController extends CrnEntryController
{
...
//        }
        ...
        if( !$error && $patient1 && $patient2 ) {
            ...
            if( !$mergedMrn1 && !$mergedMrn2 ) {
                ...
                $merged = true;
                $autoGeneratedMergeMrn = $crnUtil->autoGenerateMergeMrn($patient1);

                $patRes = $crnUtil->addGenerateMergeMrnToPatient($patient1,$autoGeneratedMergeMrn,$user);
                if( !($patRes instanceof Patient) ) {
                    $msg .= $patRes."<br>";
                    $error = true;
                }

                $patRes = $crnUtil->addGenerateMergeMrnToPatient($patient2,$autoGeneratedMergeMrn,$user);
                if( !($patRes instanceof Patient) ) {
                    $msg .= $patRes."<br>";
                    $error = true;
                }
            ...
            if( (!$mergedMrn1 && $mergedMrn2) || ($mergedMrn1 && !$mergedMrn2) ) {
                ...
                if( $mergedMrn1 ) {
                    ...
                    $newMrn = $crnUtil->createPatientMergeMrn($user,$patient2,$mergedMrn1->getField());
                ...
                if( $mergedMrn2 ) {
                    ...
                    $newMrn = $crnUtil->createPatientMergeMrn($user,$patient1,$mergedMrn2->getField());
                    if( $newMrn instanceof PatientMrn ) {
                        $merged = true;
            ...
            if( $mergedMrn1 && $mergedMrn2 ) {
                ...
                if( !$error && !$merged ) {
                    if ($patient1->hasOnlyOneMergeMrn($status) && $patient2->hasOnlyOneMergeMrn($status)) {
                        if ($mergedMrn1->getField() == $mergedMrn2->getField()) {
                            ...
                            $msg .= "Patient Records have already been merged by " . $crnUtil->obtainSameMergeMrnInfoStr($mergedMrn1, $mergedMrn2)."<br>";
                            $error = true;
                        } else {
                            ...
                            $newMrn = $crnUtil->copyOldestMrnToSecondPatient($user, $patient1, $mergedMrn1, $patient2, $mergedMrn2);
                            if ($newMrn instanceof PatientMrn) {
                                $merged = true;
                            } else {
                                $msg .= $newMrn."<br>";
                ...
                if( !$error && !$merged ) {
                    if( $crnUtil->hasSameID($patient1, $patient2) ) {
                        $msg .= "Patient Records have already been merged by " . $crnUtil->obtainSameMergeMrnInfoStr($mergedMrn1, $mergedMrn2)."<br>";
                        $error = true;
                    } else {
            ...
            if( !$error && $merged ) {
                ...
                $ids = $crnUtil->setMasterPatientRecord($patientsArr,$masterMergeRecordId,$user);
        ...
        $event = $event . $msg;
        $userSecUtil->createUserEditEvent($this->getParameter('crn.sitename'), $event, $user, $masterMergeRecordPatient, $request, $eventType);
...
//        $form2 = $this->createPatientForm($patient2);
    ...
    public function unmergePatientAjaxAction(Request $request, CrnUtil $crnUtil)
    {
        ...
        $processMasterRes = $crnUtil->processMasterRecordPatients($unmergedPatients,$masterId,$user);
        ...
        foreach( $unmergedPatients as $unmergedPatient ) {
            ...
            $processUnmergePatientRes = $crnUtil->processUnmergedPatient($unmergedPatient,$masterId,$user);
        ...
        if( count($unmergedPatients) > 0 ) {
            ...
            $event = $event . $msg;
            $userSecUtil->createUserEditEvent($this->getParameter('crn.sitename'), $event, $user, $unmergedPatients, $request, $eventType);
        }
    ...
    public function findAndEditPatientAction(Request $request) {
        ...
        if( $mrntype && $mrnid ) {
            ...
            //redirect to crn_patient_edit_by_mrn
            return $this->redirect( $this->generateUrl('crn_patient_edit_by_mrn',array('mrntype'=>$mrntype,'mrn'=>$mrnid,'show-tree-depth'=>2)) );
...
//        }
    ...
    public function updateTaskAction(Request $request, $taskId, $status)
    {
        ...
        //process.py script: replaced namespace by ::class: ['AppCrnBundle:CrnTask'] by [CrnTask::class]
            $task = $this->getDoctrine()->getRepository(CrnTask::class)->find($taskId);
            //echo "$task=".$task."<br>";
            ...
            $patient = NULL;
            $patientStr = NULL;
            $crnEntryMessage = $task->getCrnEntryMessage();
            $message = $crnEntryMessage->getMessage();
            ...
            $msg .= "Task associated with Critical Result Notification Entry ID#".$message->getOid().$patientStr." has been updated: status is set to '".$status."'".":";
