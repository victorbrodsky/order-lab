...
//        $this->generateRecLetterId($reference);
     ...
     */
    public function generateXlsxData( array $fellapps, $returnSpreadsheet=false ) {
...
//        }
     ...
     */
    public function populateRow( $sheet, FellowshipApplication $fellapp, array $headers, int $row ) {
        $userSecUtil = $this->container->get('user_security_utility');
        $instanceId = $userSecUtil->getSiteSettingParameter('instanceId');
        $logger = $this->container->get('logger');
...
//        }
        ...
        foreach ($trainings as $training) {
            ...
            if (stripos($trainingType, 'undergraduate') !== false
                //&& $data['undergraduateSchoolName'] == ''
                && array_key_exists('undergraduateSchoolName', $data)
                && $data['undergraduateSchoolName'] === ''
            ) {
                ...
                stripos($trainingType, 'graduate') !== false
                //&& $data['graduateSchoolName'] == ''
                && array_key_exists('graduateSchoolName', $data)
                && $data['graduateSchoolName'] === ''
            ) {
                ...
                //stripos($trainingType, 'medical') !== false && $data['medicalSchoolName'] == ''
                stripos($trainingType, 'medical') !== false
                && array_key_exists('medicalSchoolName', $data)
                && $data['medicalSchoolName'] === ''
            ) {
                ...
                //stripos($trainingType, 'residency') !== false && $data['residencyName'] == ''
                stripos($trainingType, 'residency') !== false
                && array_key_exists('residencyName', $data)
                && $data['residencyName'] === ''
            ) {
                ...
                if (
                    //$data['gme1Name'] == ''
                    array_key_exists('gme1Name', $data)
                    && $data['gme1Name'] === ''
                ) {
                    ...
                    //$data['gme2Name'] == ''
                    array_key_exists('gme2Name', $data)
                    && $data['gme2Name'] === ''
                ) {
...
//                if ($data['otherExperience1Name'] == '') {
                if (array_key_exists('otherExperience1Name', $data) && $data['otherExperience1Name'] === '') {
                    ...
                    //$data['otherExperience2Name'] == ''
                    array_key_exists('otherExperience2Name', $data)
                    && $data['otherExperience2Name'] === ''
                ) {
                    ...
                    //$data['otherExperience3Name'] == ''
                    array_key_exists('otherExperience3Name', $data)
                    && $data['otherExperience3Name'] === ''
                ) {
    ...
    //implement getFormId
    //googleFormId
    public function getFormId( $fellapp ) {
        $userSecUtil = $this->container->get('user_security_utility');
        $instanceId = $userSecUtil->getSiteSettingParameter('instanceId');
        $currentDateTime = new \DateTime();
     ...
     */
    public function getFirstDocumentUrl($documents) {
     ...
     */
    public function getFirstDocumentHash($documents) {
        if (!$documents || $documents->count() == 0) {
            return '';
        }
        $doc = $documents->first();
