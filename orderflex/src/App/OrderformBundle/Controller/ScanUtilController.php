...
//            $this->isGranted('ROLE_SCANORDER_SERVICE_CHIEF')
//        ) {
...
//        }
        ...
        $id = trim((string)$request->attributes->get('opt', $request->query->get('opt', $request->request->get('opt'))) );
        ...
        foreach ($parametersArr as $__setParamKey => $__setParamValue) {
    ...
    public function getAccessionTypeAction(Request $request) {
        ...
        $opt = trim((string)$request->attributes->get('opt', $request->query->get('opt', $request->request->get('opt'))) );
        $type = trim((string)$request->attributes->get('type', $request->query->get('type', $request->request->get('type'))) );
        ...
        if( $type == "single" ) {
            if( $opt && $opt != "undefined" ) {
                ...
                $query->setParameter('type', 'default')->setParameter('typetma', 'TMA')->setParameter('user', $user);
            }
        } else {
            if( $opt && $opt != "undefined" ) {
    ...
    public function getMrnTypeAction(Request $request) {
        ...
        $opt = trim((string)$request->attributes->get('opt', $request->query->get('opt', $request->request->get('opt'))) );
        $type = trim((string)$request->attributes->get('type', $request->query->get('type', $request->request->get('type'))) );
        $exception = trim((string)$request->attributes->get('exception', $request->query->get('exception', $request->request->get('exception'))) );
        ...
        if( $opt && $opt != "undefined" ) {
            ...
            $query->setParameter('type', 'default')->setParameter('user', $user);
        }
    ...
    public function getOptionalUserAction(Request $request) {
        ...
        $holderId = trim((string)$request->attributes->get('opt', $request->query->get('opt', $request->request->get('opt'))) ); //associated object ProjectTitleTree id
        ...
        if( $holderId && $holderId != "undefined" ) {
            $query = $em->createQueryBuilder()->from($fromClass, 'list')->select("userWrappers.id as id, (CASE WHEN userWrappersUser.id IS NULL THEN userWrappers.name ELSE userWrappers.name+' - '+userWrappersUserInfos.displayName END) as text")->leftJoin("list.userWrappers", "userWrappers")->leftJoin("userWrappers.user", "userWrappersUser")->leftJoin("userWrappersUser.infos", "userWrappersUserInfos")->where("list.id = :holderId AND (userWrappers.type = :type OR userWrappers.type = :type2)")->orderBy("list.orderinlist", "ASC")->setParameter('holderId', $holderId)->setParameter('type', 'default')->setParameter('type2', 'user-added');
    ...
    public function getReturnLocationAction(Request $request) {
        ...
        $providerid = trim((string)$request->attributes->get('providerid', $request->query->get('providerid', $request->request->get('providerid'))) );
        $proxyid = trim((string)$request->attributes->get('proxyid', $request->query->get('proxyid', $request->request->get('proxyid'))) );
