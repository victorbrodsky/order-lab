...
class GoogleFormConfigController extends OrderAbstractController
{
    ...
    /**
     * @Route("/form-status-and-appearance/edit", name="resapp_google_form_config_edit", methods={"GET", "PUT"})
     * @Route("/form-status-and-appearance/show", name="resapp_google_form_config_show", methods={"GET", "PUT"})
    ...
    /**
     * @Route("/google-form-config-update-drive", name="resapp_google_form_config_update_drive", methods={"GET"})
     */
    public function GoogleFormConfigUpdateDriveAction(Request $request) {
        ...
        $configFile = $googlesheetmanagement->findConfigFileInFolder($service, $configFileFolderIdResApp, "config.json");
        if( $configFile ) {
            $configFile->getId();
        } else {
        ...
        $newDescription = "config file for residency application generated by " . $user . " from server " . $request->getSchemeAndHttpHost();
...
//    }
    ...
    function updateFileContent($service, $fileId, $newTitle, $newDescription, $newMimeType, $content, $newRevision) {
        try {
            ...
            // File's new content.
            //$newFileName = "C:\\Users\\ch3\\Documents\\MyDocs\\WCMC\\ORDER\\scanorder\\Scanorders2\\src\App\\ResAppBundle\\Util\\GoogleForm\\"."config.json";
