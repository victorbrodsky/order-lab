...
use App\ResAppBundle\Util\CustomDefaultServiceRequest;
...
//        $serviceRequest = $this->container->get('resapp_customd_defaultservicerequest');
//        $serviceRequest->setAccessRequest($accessToken);

        //TODO:  Cannot autowire service "App\ResAppBundle\Util\CustomDefaultServiceRequest": argument "$accessToken" of method "__construct()" is type-hinted "string", you should configure its value explicitly.
...
//        $serviceRequest::CURLOPT_SSL_VERIFYPEER = false;
    ...
    public function deleteImportedApplicationAndUploadsFromGoogleDrive( $worksheet, $rowId ) {
        ...
        if( !$rowId ) {
            $logger->warning('Residency Application Google Form ID does not exists. rowId='.$rowId);
        }
...
//            if( $type && $type == 'excel' ) {
//                $resappSpreadsheetType = $this->em->getRepository('AppUserdirectoryBundle:DocumentTypeList')->findOneByName('Residency Application Spreadsheet');
//            } else {
//                $resappSpreadsheetType = $this->em->getRepository('AppUserdirectoryBundle:DocumentTypeList')->findOneByName('Residency Application Document');
//            }
    ...
    //ResidencyApplication -> Responses -> RecommendationLetters -> (RecommendationLetterUploads, Spreadsheets, Template)
...
//        //$spreadsheet = $spreadsheetFeed->getByTitle('Resapp-test');
...
//    }
    ...
    public function authenticationP12Key() {
        ...
        $client_email = $userSecUtil->getSiteSettingParameter('clientEmailResApp');

        //$pkey = __DIR__ . '/../Util/ResidencyApplication-f1d9f98353e5.p12';
        $pkey = $userSecUtil->getSiteSettingParameter('p12KeyPathResApp');
        if( !$pkey ) {
            $logger->warning('p12KeyPathResApp/credentials.json is not defined in Site Parameters. File='.$pkey);
        }

        //$user_to_impersonate = 'olegivanov@pathologysystems.org';
        $user_to_impersonate = $userSecUtil->getSiteSettingParameter('userImpersonateEmailResApp');

        //echo "pkey=".$pkey."<br>";
        $private_key = file_get_contents($pkey); //notasecret

        $googleDriveApiUrlResApp = $userSecUtil->getSiteSettingParameter('googleDriveApiUrlResApp');
        if( !$googleDriveApiUrlResApp ) {
            throw new \InvalidArgumentException('googleDriveApiUrlResApp is not defined in Site Parameters.');
        }
        ...
        //$scopes = array($googleDriveApiUrlResApp); //'https://www.googleapis.com/auth/drive'
        $scopes = $googleDriveApiUrlResApp;
...
//        }
    ...
    public function getClient() {
        ...
        if( !$user_to_impersonate ) {
            throw new \InvalidArgumentException('userImpersonateEmailResApp is not defined in Site Parameters.');
        }

        $credentialsJsonFile = $userSecUtil->getSiteSettingParameter('p12KeyPathResApp');
        ...
        $client->setApplicationName('Residency Applications');
...
//        ));
    ...
    function getClient2() {
        ...
        $scopes = $userSecUtil->getSiteSettingParameter('googleDriveApiUrlResApp');
        if( !$scopes ) {
    ...
    function downloadFile($service, $file, $type=null) {
        $logger = $this->container->get('logger');
        if( $type && ($type == 'Residency Application Spreadsheet' || $type == 'Residency Application Backup Spreadsheet' || $type == 'Residency Recommendation Letter Spreadsheet') ) {
    ...
    public function getConfigOnGoogleDrive() {
        ...
        if( $this->container->get('security.authorization_checker')->isGranted('ROLE_RESAPP_ADMIN') === false ) {
