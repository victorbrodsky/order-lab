...
//        $userSecUtil->createUserEditEvent($sitename,$resStr,$user,null,$request,'Restore Backup Database');
        ...
        $userServiceUtil = $this->container->get('user_service_utility');
        $res = $userServiceUtil->dbManagePython($networkDrivePath,'restore',$sync=false,$backupFileName,$sendStatusEmail='yes',$env); //Use python script pg_restore
        //exit($res);
        ...
        //TODO: run after asynchronous restore is done.
        //In pytrhon restore script
...
//        }
    ...
    //trigger-post-db-updates: secure this url, send an hash id generated by the server
    // (use parameters.yml secret? or Shared Secret Token)
    //    python:
    //    headers = {'X-Callback-Token': 'your-secure-token'}
    //    requests.post(callback_url, json=payload, headers=headers, verify=False)
    //    php:
    //    $token = $request->headers->get('X-Callback-Token');
    //    if ($token !== $_ENV['CALLBACK_SECRET']) {
    //        return new Response('Unauthorized', Response::HTTP_UNAUTHORIZED);
    //    }
    //Or use Signed URLs (Advanced) https://symfony.com/doc/current/routing.html#signing-uris:
    // $url = $urlGenerator->generate('your_callback_route', [], UrlGeneratorInterface::ABSOLUTE_URL | UrlGeneratorInterface::SIGNATURE);

    //http://127.0.0.1/directory/send-confirmation-email/
    #[Route(path: '/trigger-post-db-updates/', name: 'employees_trigger_post_db_updates', methods: ['POST'])]
    public function triggerPostDbUpdatesAction( Request $request )
    {
        //TODO: add Rate Limiter
        //$limiter = $anonymousApiLimiter->create($request->getClientIp());
        // the argument of consume() is the number of tokens to consume
