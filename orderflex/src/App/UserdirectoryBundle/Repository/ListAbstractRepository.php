...
class ListAbstractRepository extends EntityRepository {
    ...
    public function convertStrToObject( $name, $objectParams, $user, $parentFieldName = null, $parentId=null ) {
        ...
        //$entity = $this->getEntityManager()->getRepository($objectParams['fullBundleName'].':'.$objectParams['className'])->findOneBy( $criterions );
        $entity = $this->getEntityManager()->getRepository($objectParams['fullClassName'])->findOneBy( $criterions );
    ...
    public function createNewListEntity( $objectParams, $name, $user ) {
        ...
        //get max orderinlist
        //$query = $this->getEntityManager()->createQuery('SELECT MAX(c.orderinlist) as maxorderinlist FROM '.$objectParams['fullBundleName'].':'.$objectParams['className'].' c');
        $query = $this->getEntityManager()->createQuery('SELECT MAX(c.orderinlist) as maxorderinlist FROM '.$entityClass.' c');
