...
//    }
    ...
    public function LocalAuthentication($token) {
        ...
        if( $user ) {
            ...
            $this->logger->notice("Local Authentication: local user password=".$user->getPassword()); //testing
    ...
    //NOT USED: keep it for a reference
...
//            }
            ...
            if (str_contains((string)$origLdapBindDN, 'dc=wcmc-ad')) {
                ...
                //for generic ldap (i.e. Wustl/Washu)
    ...
    {
        $this->logger->notice('connectToLdap: LDAPHost='.$LDAPHost.', LDAPPort='.$LDAPPort);
        if( empty($LDAPHost) ) {
