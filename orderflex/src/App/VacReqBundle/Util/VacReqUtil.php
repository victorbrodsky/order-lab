...
//                echo $count.": with effort $effort, thisTotalAccruedDays=$thisTotalAccruedDays <br>";
    ...
    //$yearRange=2024-2025
    //Return only the latest employment period (by hireDate) within the academic year range
    ...
    //        }
        ...
        //Return only the latest employment period.
        //Periods are ordered by hireDate ASC; walk backwards to skip trailing periods without a hire date.
        $latestEmplPeriod = NULL;
        for( $i = count($emplPeriods)-1; $i >= 0; $i-- ) {
            if( $emplPeriods[$i]->getHireDate() ) {
                $latestEmplPeriod = $emplPeriods[$i];
                break;
            }
        }

        if( $testing ) {
            echo "latestEmplPeriod=" . ($latestEmplPeriod ? $latestEmplPeriod->getVacReqData() : "none") . "<br>";
            echo "================= <br>";
        }
    ...
    public function calculateAdjustedByEmplPeriodParam( $user, $accruedMonths, $vacationAccruedDaysPerMonth, $emplPeriod=NULL ) {
        //$accruedDays = 0;
    ...
    //Calculate number of month between the start/end dates (user independent function)
    //$yearRange=2024-2025
    public function getTotalAccruedMonths( $yearRangeStr, $startDate=NULL, $endDate=NULL, $testing=FALSE ) {
...
//            $academicYearEndDateStrThis = $currentYear."-".$academicYearEndDateStrThis;
        ...
        //interval: between S and 30June2025 (end of academic year)
        if(
            $totalAccruedMonths === NULL
            && $startDate
            && $startDate > $academicYearStartDate && $startDate < $academicYearEndDate
...
//        }
        ...
        $totalAccruedMonths = $this->getTotalAccruedMonths($yearRangeStr,$startDate,$endDate,$testing=TRUE);
