  ...
  then
    if [ "$multitenant" = "haproxy" ]
      then
        ...
        #doctl compute ssh "packer-1745428562" --ssh-key-path ./sshkey --ssh-command 'bash /srv/order-lab-homepagemanager/packer/install-certbot.sh view.online installcertbot oli2002@med.cornell.edu haproxy' -o StrictHostKeyChecking=no
