...
###### Create ssh keys ######
echo "*** Create ssh keys stored in file sshkey, later we can use this ssh private key to manipulate the droplet using doctl with option --ssh-key-path path-to-my-ssh-private-key ***"
...
if [ "$sslcertificate" = "installcertbot" ] && [ -n "$domainname" ] && [ -n "$email" ]
  then
    if [ "$multitenant" = "haproxy" ]
      then
        echo -e ${COLOR} Run bash script install-certbot.sh via ssh. IMAGENAME="$IMAGENAME", domainname="$domainname", sslcertificate="$sslcertificate", email="$email" multitenant="$multitenant" ${NC}
        ...
        #yes yes | doctl compute ssh "packer-1745521900" --ssh-key-path ./sshkey --ssh-command 'bash /srv/order-lab-homepagemanager/packer/install-certbot.sh view.online installcertbot oli2002@med.cornell.edu haproxy' -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null
        yes yes | doctl compute ssh "$IMAGENAME" --ssh-key-path ./sshkey --ssh-command 'bash /srv/order-lab-homepagemanager/packer/install-certbot.sh $domainname $sslcertificate $email $multitenant' -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null
        ...
        echo -e "${COLOR} Restart HaProxy via ssh 'sudo systemctl restart haproxy' ${NC}"
        doctl compute ssh "$IMAGENAME" --ssh-key-path ./sshkey --ssh-command 'sudo systemctl restart haproxy'
        echo -e "${COLOR} Restart php-fpm via ssh 'sudo systemctl restart php-fpm' ${NC}"
        doctl compute ssh "$IMAGENAME" --ssh-key-path ./sshkey --ssh-command 'sudo systemctl restart php-fpm'
    else
...
echo -e "${COLOR} doctl compute ssh $IMAGENAME --ssh-key-path ./sshkey  ${NC}"
